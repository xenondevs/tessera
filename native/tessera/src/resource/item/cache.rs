...
impl ItemCache {
    ...
    pub async fn prime(&self, rm: &ResourceManager, models: &ModelCache, ids: &[ResourceId], diag: &Diagnostics) {
        ...
        let parsed = rayon_batch(ids.iter().cloned().zip(bytes.into_iter()).collect(), |(id, bytes)| {
            let bytes = bytes
                .map(|b| b.into_owned())
                .ok_or_else(|| ItemError::NotFound(id.clone()));
            let item = bytes.and_then(|mut b| {
                simd_json::serde::from_slice::<RawItem>(&mut b).map_err(|e| ItemError::Parse(id.clone(), e.to_string()))
            });
    ...
    fn resolve(models: &ModelCache, id: &ResourceId, raw: RawItemModel, diag: &Diagnostics) -> ItemModel {
        ...
        match raw {
            ...
            RawItemModel::RangeDispatch { property, scale, entries, fallback, .. } => ItemModel::RangeDispatch {
                property,
                scale,
                entries: entries
                    .into_iter()
            ...
            RawItemModel::Select { property, cases, fallback, .. } => ItemModel::Select {
                ...
                cases: cases
                    .into_iter()
                    .map(|case| (case.when.into_boxed(), Self::resolve(models, id, case.model, diag)))
                    .collect(),
                fallback: fallback.map(|model| Box::new(Self::resolve(models, id, *model, diag))),
            },
